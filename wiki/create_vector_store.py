...
def create_vector_store(
    ...
    Parameters:
    -----------
    csv_file: Wikipedia CSV file (จาก prepare_wikipedia_data.py)
    vector_store_path: โฟลเดอร์เก็บ vector store
    collection_name: ชื่อ collection
    embedding_model: HuggingFace embedding model
        - BAAI/bge-m3: multilingual, ดีสำหรับภาษาไทย (แนะนำ) ~2.3GB
        - intfloat/multilingual-e5-base: เล็กกว่า เร็วกว่า ~1.1GB
        - sentence-transformers/paraphrase-multilingual-mpnet-base-v2 ~1GB
    ...
    if not os.path.exists(csv_file):
        ...
        print("💡 กรุณารัน: python prepare_wikipedia_data.py ก่อน")
        return
    ...
    # ประมาณการเวลา
    est_minutes = total_rows / 1000  # ~1000 docs/min with mps
    print(f"⏱️  Estimated time: ~{est_minutes:.0f} minutes")
    print(f"💾 Estimated vector store size: ~{file_size * 2:.0f} MB")
    print()
    
    print("🔄 Creating vector store...")
    print("💡 Tips:")
    print("   - ครั้งแรกจะ download embedding model (~500MB-2GB)")
    print("   - ถ้า out of memory: ลด --batch-size (16 หรือ 8)")
    print("   - Mac M1/M2: device=mps เร็วกว่า cpu มาก")
    print("   - ถ้ายกเลิก: สามารถรันใหม่ได้ (จะ resume)")
    print()
    ...
    try:
        ...
        # เช็คขนาด vector store
        if os.path.exists(vector_store_path):
            import shutil
            vs_size = sum(f.stat().st_size for f in Path(vector_store_path).rglob('*') if f.is_file())
            print(f"💾 Vector store size: {vs_size / 1024 / 1024:.2f} MB")
        ...
        print("💡 คุณสามารถรันคำสั่งเดิมอีกครั้งเพื่อ resume ได้")
        ...
        print("   4. CSV format error → ตรวจสอบ CSV ด้วย head -n 5")
...
ตัวอย่างการใช้งาน:
  ...
  python create_vector_store.py \\
    --csv-file wikipedia_thai_vectorrm.csv
...
⚠️ หมายเหตุ:
  ...
  - สามารถยกเลิกและรันใหม่ได้ (resume)
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Wikipedia CSV file (จาก prepare_wikipedia_data.py)"
    )
    parser.add_argument(
        "--vector-store-path",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Collection name (default: wikipedia_thai)"
    )
    parser.add_argument(
        "--embedding-model",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Device for embedding (default: mps for Mac M1/M2)"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
